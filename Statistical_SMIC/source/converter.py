...
class ConvertDlg( QDialog, converter_ui.Ui_converter):
    ...
    def on_Input_button_clicked(self):
        source_file_path = QFileDialog.getOpenFileName(self, self.tr("Select data file"), QString(), self.tr("Data File(*.dat *.data data);;All File(*.*)"))
...
class Convert(QThread):
    ...
    def run(self):
        try:
            smic.gui_itf(self,self.in_path,self.out_path,self.sigma,self.ratio,self.rename)
        except:
